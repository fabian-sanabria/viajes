...
const MiViaje = () => {
  const { contratacion, dispacth2 ,referencia,totalApagar,setTotalApagar} = useContext(Contexto);
  return (
    <>
...
{(contratacion.length<1)&&  <div className="aviso">
      ...
      Haz click en <Link to={"/cp"}>Capital y patagonia</Link> o <Link to={"/no"}>Norte o este</Link> para ver nuestras ofertas
    </div>}
   
      {contratacion.map((mapa) => (
        <div key={mapa.sitio}>
          <button className="boton anular-viaje"
            onClick={() => {
